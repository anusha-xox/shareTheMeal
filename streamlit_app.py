...
if start_date and end_date:
    if start_date <= end_date:
        ...
        response = requests.post(f"http://127.0.0.1:5000/predict_wastage_food",
                                 data={'date_start_input': start_date.isoformat(),
                                       'date_end_input': end_date.isoformat(), 'selected_hotel': selected_hotel})
        ...
        if response.status_code == 200:
            if selected_hotel == "Hotel-1":
                ...
                op_df = pd.DataFrame(
                    {'Date': pd.date_range(start=start_date, end=end_date), 'ARIMA_Predictions': pred_list})
